...
import { Interface } from './extract-fields';
...
const getVariations = (
    ...
    interfaces[interfaceName].props.forEach(
        prop => prop.choice && vs.add(`${interfaceName}${prop.choice}`),
    );
    return [...vs];
};
...
const getPrimitive = (
    ...
    if (!prop?.type) return;
    return isPrimitive(prop.type)
        ? prop.type
        : getPrimitive(prop?.type!, schema);
};
...
const buildInterfaces = (interfaces: Record<string, Interface>): string => {
    const declarations = Object.entries(interfaces)
        .map(([name, _interface]) => {
            const variations =
                Number(
                    max(
                        _interface.props
                            .filter(prop => !Number.isNaN(prop.choice))
            ...
            return range(0, variations).map(variation => {
                const propSignatures = _interface.props
                    .map(prop => {
                        ...
                        if (!isNil(prop.choice) && prop.choice !== variation)
                            return;
                        ...
                        if (prop.values?.length) {
                            ...
                            if (!prop.reference && !prop.type) {
                                throw new Error(
                                    `A Prop should have either a "reference" or a "type" field. Prop ${prop.name} has none.`,
                                );
                            }
                            const resolvedType = resolveType(
                                (prop.reference || prop.type)!,
                                interfaces,
                            );
                            if (Array.isArray(resolvedType)) {
                                ...
                                hasVariations(resolvedType, interfaces)
                            ) {
                                typeNode = ts.factory.createUnionTypeNode(
                                    getVariations(resolvedType, interfaces).map(
                                        refName =>
                ...
                if (_interface.extend) {
                    ...
                    if (Array.isArray(type))
                        throw new Error(
                            `I'm not prepared for this. Should this happen?`,
                        );

                    if (isPrimitive(type)) {
                        // [known issue 002]
                        // interface extends a primitive: edge case where an element has children (elements or attrs) and a value itself
                        ...
                        propSignatures.push(
                            ts.factory.createPropertySignature(
                                undefined,
                                normalizePropName(_interface.name),
                                undefined,
                                mapToTypeNode(type),
                            ),
                        );
        ...
        .flat()
        .filter(Boolean) as InterfaceDeclaration[];
